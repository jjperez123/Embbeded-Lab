...
int count = 0;
int value = 0;
void ConfigureAdc_temp(); // configure adc to read from sensor one
void ConfigureAdc_temp2(); // configure adc to read from sensor 2
void uart_init(void); // configures uart
...
void main(void)
{
    ...
    while(1){
                ...
                itoa(value,result,10);                   // convert temperature reading to Fahrenheit
                acount =0;
                while(result[acount]!='\0')
                  {
                   while((IFG2 & UCA0TXIFG)==0);                  //Wait Unitl the UART transmitter is ready
